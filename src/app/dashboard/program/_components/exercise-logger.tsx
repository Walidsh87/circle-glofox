...
import type { SetLog } from '@/lib/program-log'
...
const daySummary = (sets: SetLog[]) => sets.map((s) => `${kg(s.weight_grams)}×${s.reps ?? '—'}`).join(', ')
...
export function ExerciseLogger({ exercise, today, videoSlug }: { exercise: LoggableExercise; today: string; videoSlug?: string | null }) {
  ...
  const defaultRows = (): Row[] => {
    ...
    const prefillWeight = exercise.load ? String(exercise.load.barKg) : ''
    const prefillReps = /^\d+$/.test(exercise.reps) ? exercise.reps : ''
    return Array.from({ length: Math.min(count, 50) }, () => ({ weight: prefillWeight, reps: prefillReps }))
  }
      ...
      {open && (
        <div className="mt-2 flex flex-col gap-1.5 rounded-lg border border-line bg-surface-2 px-3 py-2.5">
          ...
          <div className="flex gap-2 pt-1">
            <button type="button" className={btn} onClick={() => setRows((p) => [...p, { weight: '', reps: '' }])}>+ set</button>
        ...
        <div className="mt-1.5">
          {!showHistory ? (
            <button type="button" className="text-[11px] text-ink-3 underline" onClick={() => setShowHistory(true)}>
              Last {last.date}: {daySummary(last.sets)} · history ({exercise.logDays.length})
            </button>
          ) : (
            <div className="flex flex-col gap-1">
              {exercise.logDays.map((d) => (
                <div key={d.date} className="flex items-center justify-between gap-2 text-[11.5px] text-ink-3">
                  <span><span className="font-mono">{d.date}</span> — {daySummary(d.sets)}</span>
