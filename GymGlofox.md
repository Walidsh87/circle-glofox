...
| **Migrations** | 008–044 ✅ in repo. 023–027 applied to prod ✅. ⚠️ **Pending in Supabase: `028`–`044`** (028 TV; 029 WOD scaling; 030 retention outreach; 031 class waitlist; 032 Committed Club; 033 membership freeze + cron-fn frozen-skip; 034 member safety/medical cols; 035 membership_plans catalog + plan_id; 036 trial cols; 037 member_tags; 038 households + profiles.household_id; 039 booking-policy cols; 040 skill_levels; 041 broadcasts + recipients + profiles.marketing_opt_out/unsubscribe_token; 042 email campaigns — broadcasts.body_blocks/template_id, recipients.resend_id/opened_at/clicked_at, email_templates; 043 automations + automation_runs ledger; 044 sequences + sequence_enrollments + sequence_sends). |
| **Next session priority** | Run migrations 028–044 in Supabase. ⚙️ set `ANTHROPIC_API_KEY` in Vercel for #16. ⚙️ enable Resend open/click tracking + register webhook `/api/webhooks/resend` (events email.opened/clicked/bounced/complained) + set `RESEND_WEBHOOK_SECRET` in Vercel for #41 analytics. ⚙️ Vercel crons `/api/cron/automations` (06:00) + `/api/cron/sequences` (06:15) use existing `CRON_SECRET` — no new env. **Tiers 3 + 4 COMPLETE; Tier 5 — #43 broadcasts + #41 email campaigns + #37 automation builder + #38 lifecycle pipeline board + #44 automated sequences done (5/13; #38 onboarding/offboarding checklists deferred).** Next in Tier 5: #42 SMS, #39 WhatsApp, or #45 lead-capture widget. |
