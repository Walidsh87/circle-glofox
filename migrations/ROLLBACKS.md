...
Reverse procedures for migrations `008`–`083` (referenced by the DR runbook, `docs/runbooks/disaster-recovery.md`).
...
### 078_api_keys
```sql
drop table if exists api_keys;   -- ⚠️ deletes issued public-API credentials
```

### 077_invoice_charge_ref_unique
```sql
drop index if exists idx_invoices_provider_charge_ref;  -- backstop only; no data lost
```
