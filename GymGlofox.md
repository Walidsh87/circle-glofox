...
| **v2 Tier 2–13 (~95 items)** | 5 ✅ (#11 WOD programming + batch import, #12 auto-PR detection — lift PRs, #13 coach prep view, #23 1RM charts, #25 activity feed) · #21 mobile API ⬜ (deferred) · rest ⬜ |
| **Migrations** | 008–026 ✅ in repo (… 024 workout_templates · 025 lift is_pr · 026 coach notes). 023 + 024 applied to prod ✅. ⚠️ **Run 025 + 026 in Supabase before those features go live** (025: auto-PR `is_pr`; 026: `athlete_coach_notes` for the prep view). |
| **Next session priority** | Run migrations 025 + 026 in Supabase, then resume v2 Tier 2 (next wedge candidates: #16 AI parser — feeds the batch-import preview→commit pipeline; #12 WOD/benchmark PRs as a follow-on to lift PRs; #14 whiteboard/TV mode) |
...
12. 🟡 `[Wedge]` **Barbell-lift / strength-tracking engine with auto-PR detection** (extend current 1RM tracking) — ✅ **lift PRs done**: `saveLift` flags `is_pr` on new-max saves (migration 025), form celebration + chart/table 🏆 + box-wide activity-feed entry (display-only); RLS exposes only PR rows. WOD/benchmark PRs = follow-on. Spec `…auto-pr-detection-design.md`.
