...
import { currentBests } from '@/lib/skill-bests'
...
// their current data (1RM / skill best / attendance). Box-scoped; RLS additionally
...
export async function loadGoalsData(
  ...
  const [{ data: goalRows }, { data: planRows }, { data: liftRows }, { data: bestRows }] = await Promise.all([
    ...
    supabase.from('athlete_skill_bests').select('skill_key, value').eq('athlete_id', athleteId).eq('box_id', boxId),
  ])
  ...
  const bests = currentBests((bestRows ?? []) as { skill_key: string; value: number }[])
  ...
  const goals = allGoals.map((g) => {
    ...
    return {
      ...
      progress: goalProgress(g, {
        ...
        bestValue: g.skill_key ? (bests[g.skill_key] ?? null) : null,
        attendanceCount,
      }),
    }
  })
