...
`../seed-wod-schedule-programs.sql`, `../seed-movement-videos.sql` and `../seed-program-days.sql` are the exceptions — they're meant to run against the real box in prod (deliberately, by hand). They add programming content only (WODs, class schedule, coach training programs + their day sessions, movement demo videos) for the real coaches/athletes; no fake members/billing/leads. Idempotent, safe to re-run.
