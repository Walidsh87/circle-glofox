...
// Staff RLS client: coach in box b1. The athlete's paid membership is read via the SERVICE client
// (mig 090), so it's configured on svc below (skips the credit path).
function rls() {
...
const PAID = { payment_status: 'paid', end_date: null, last_paid_date: '2026-06-01' }
...
test('a check-in that lands exactly on a milestone records the achievement', async () => {
  ...
  const svc = makeSupabaseMock({ results: { memberships: { data: [PAID], error: null }, bookings: { data: Array(25).fill(ci('2026-06-01')), error: null } } })
...
test('no crossing → no achievement insert', async () => {
  serverCreate.mockResolvedValue(rls())
  const svc = makeSupabaseMock({ results: { memberships: { data: [PAID], error: null }, bookings: { data: Array(10).fill(ci('2026-06-01')), error: null } } })
  serviceCreate.mockReturnValue(svc)
