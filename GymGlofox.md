...
| **v2 Tier 2–13 (~95 items)** | 7 ✅ (#11 WOD programming + batch import, #12 auto-PR (lift + WOD), #13 coach prep view, #14 whiteboard/TV mode, **#16 AI workout parser**, #23 1RM charts, #25 activity feed) · #21 mobile API ⬜ (deferred) · rest ⬜ |
| **Migrations** | 008–028 ✅ in repo. 023–027 applied to prod ✅. ⚠️ **Only `028_tv_token.sql` still pending in Supabase** (`boxes.tv_token` for the public TV board). #16 added NO migration. |
| **Next session priority** | Build **#17 multiple programming tracks** (Rx/Scaled/Beginner — queued; structural, touches `workouts` model + scoring/PRs/leaderboard/TV). Run 028 in Supabase. ⚙️ to use #16, set `ANTHROPIC_API_KEY` in Vercel + local `.env`. Tier 2 programming layer: only #17 + #15 (deferred) left. |
