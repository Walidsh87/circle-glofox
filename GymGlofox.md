...
| **v2 Tier 2–13 (~95 items)** | 8 ✅ (#11 WOD programming + batch import, #12 auto-PR (lift + WOD), #13 coach prep view, #14 whiteboard/TV mode, #16 AI workout parser, **#17 scaling variations**, #23 1RM charts, #25 activity feed) · #21 mobile API ⬜ (deferred) · rest ⬜. **Tier 2 programming layer = only #15 marketplace left (deferred, v3-scale).** |
| **Migrations** | 008–029 ✅ in repo. 023–027 applied to prod ✅. ⚠️ **Pending in Supabase: `028_tv_token.sql` + `029_workout_scaling.sql`** (028 = TV board's `boxes.tv_token`; 029 = `workouts.scaling`). |
| **Next session priority** | Run migrations 028 + 029 in Supabase. ⚙️ to use #16, set `ANTHROPIC_API_KEY` in Vercel + local `.env`. Tier 2 programming layer is effectively done — next candidates: Tier 3 (Retention: #18 at-risk scoring, #19 KPI dashboard, #20 gamification, #24 timer, #26 waitlist) or Tier 4 (Membership depth). |
